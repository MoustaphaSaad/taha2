...
{
	void FrameParser::pushPayload(Span<const std::byte> payload) {
	...
	Result<bool> FrameParser::step(const std::byte *ptr, size_t size) {
	...
	Result<bool> FrameParser::consume(const std::byte* ptr, size_t size)
	{
