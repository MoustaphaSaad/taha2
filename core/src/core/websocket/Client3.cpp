...
namespace core::websocket
{
	...
	public:
		ReadMessageThread3(EventLoop2* loop, Client3* client, EventSocket2 socket, size_t maxMessageSize, Log* log, Allocator* allocator)
			: EventThread2(loop),
			  m_allocator(allocator),
			  m_log(log),
			  m_socket(socket),
	...
	HumanError Client3::startReadingMessages(const Shared<EventThread2>& handler)
	{
		...
		loop->startThread<ReadMessageThread3>(loop, this, m_socket, m_maxMessageSize, m_log, m_allocator);
		return {};
	}
}