...
namespace fin
{
	...
	public:
		...
		core::HumanError handle(core::Event* event) override
		{
			if (auto messageEvent = dynamic_cast<core::websocket::MessageEvent*>(event))
			{
				...
				{
					return messageEvent->client()->writeBinary(
						core::Span<const std::byte>{messageEvent->message().payload});
				}
				else
				{
		...
		core::Allocator* allocator)
	{
		...
		auto file = core::File::open(
			allocator,
			lockInfo.portFilePath(),
			core::File::IO_MODE_WRITE,
			core::File::OPEN_MODE_CREATE_OVERWRITE,
			core::File::SHARE_MODE_NONE);
		if (file == nullptr)
		{
			return core::errf(
				allocator, "failed to write listening port to port file '{}'"_sv, lockInfo.portFilePath());
		}
		core::strf(file.get(), "{}"_sv, server->listeningPort());

		return core::unique_from<Server>(
			allocator,
			std::move(eventLoop),
			std::move(server),
