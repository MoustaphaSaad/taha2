...
namespace core::ws
{
	Result<Server> Server::connect(StringView url, size_t maxHandshakeSize, size_t maxMessageSize, Log* log, Allocator* allocator)
	{
	...
	Result<Client> Server::accept()
	{
		while (m_socket->listen())
		{
			...
			auto clientResult = Client::acceptFromServer(std::move(clientSocket), m_maxHandshakeSize, m_maxMessageSize, m_log, m_allocator);
