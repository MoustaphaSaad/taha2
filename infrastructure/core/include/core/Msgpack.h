...
namespace core::msgpack
{
	...
	inline HumanError msgpack(Writer& writer, const Array<T>& value)
	{
		...
		{
			if (auto err = writer.write_uint8(0xdc))
			{
				return err;
			}
		...
		{
			if (auto err = writer.write_uint8(0xdd))
			{
				return err;
			}
	...
	inline HumanError msgpack(Writer& writer, const T (&arr)[N])
	{
		...
		else if (N <= UINT16_MAX)
		{
			...
			if (auto err = writer.write_uint16((uint16_t)N))
			{
				return err;
			}
		}
		else if (N <= UINT32_MAX)
		{
			...
			if (auto err = writer.write_uint32((uint32_t)N))
			{
				return err;
			}
		}
	...
	inline HumanError msgpack(Writer& writer, const Map<TKey, TValue, THash>& value)
	{
		...
		{
			if (auto err = writer.write_uint8(0xde))
			{
				return err;
			}
			if (auto err = writer.write_uint16((uint16_t)value.count()))
		...
		{
			if (auto err = writer.write_uint8(0xdf))
			{
				return err;
			}
			if (auto err = writer.write_uint32((uint32_t)value.count()))
		...
		for (const auto& element: value)
		{
			if (auto err = msgpack(writer, element.key))
			{
				return err;
			}
			if (auto err = msgpack(writer, element.value))
			{
				return err;
			}
		}
	...
	inline HumanError msgpack(Writer& writer, uint8_t value)
	{
		return msgpack(writer, (uint64_t)value);
	}
	inline HumanError msgpack(Writer& writer, uint16_t value)
	{
		return msgpack(writer, (uint64_t)value);
	}
	inline HumanError msgpack(Writer& writer, uint32_t value)
	{
		return msgpack(writer, (uint64_t)value);
	}
	inline HumanError msgpack(Writer& writer, int8_t value)
	{
		return msgpack(writer, (int64_t)value);
	}
	inline HumanError msgpack(Writer& writer, int16_t value)
	{
		return msgpack(writer, (int64_t)value);
	}
	inline HumanError msgpack(Writer& writer, int32_t value)
	{
		return msgpack(writer, (int64_t)value);
	}
	inline HumanError msgpack(Writer& writer, const String& value)
	{
		return msgpack(writer, (StringView)value);
	}
	inline HumanError msgpack(Writer& writer, const Buffer& value)
	{
		return msgpack(writer, value.data(), value.count());
	}
	...
	{
		T operator()(Reader& reader) const
		{
			return T{};
		}
	};
	...
	{
		String operator()(Reader& reader) const
		{
			return String{reader.allocator()};
		}
	};
	...
	{
		Buffer operator()(Reader& reader) const
		{
			return Buffer{reader.allocator()};
		}
	};
	...
	{
		Array<T> operator()(Reader& reader) const
		{
			return Array<T>{reader.allocator()};
		}
	};
	...
	{
		Map<TKey, TValue, THash> operator()(Reader& reader) const
		{
			return Map<TKey, TValue, THash>{reader.allocator()};
		}
	};
	...
	inline HumanError msgpack(Reader& reader, Map<TKey, TValue, THash>& res)
	{
		...
		for (size_t i = 0; i < count.value(); ++i)
		{
			...
			if (auto err = msgpack(reader, key))
			{
				return err;
			}

			auto value = DefaultConstruct<TValue>{}(reader);
			if (auto err = msgpack(reader, value))
			{
				return err;
			}
	...
	public:
		...
		Field(StringView name, T& value)
			: m_name{name},
			  m_value{(void*)&value}
		{
			m_write = [](Writer& writer, const void* value) -> HumanError { return msgpack(writer, *(const T*)value); };
			m_read = [](Reader& reader, void* value) -> HumanError { return msgpack(reader, *(T*)value); };
		}
		...
		HumanError write(Writer& writer) const
		{
			return m_write(writer, m_value);
		}
		HumanError read(Reader& reader) const
		{
			return m_read(reader, m_value);
		}
	};
	...
	inline HumanError struct_fields(Writer& writer, TField&&... fields)
	{
		...
		else if (count <= UINT16_MAX)
		{
			...
			if (auto err = writer.write_uint16((uint16_t)count))
			{
				return err;
			}
		}
		else if (count <= UINT32_MAX)
		{
			...
			if (auto err = writer.write_uint32((uint32_t)count))
			{
				return err;
			}
		}
		...
		{
			if (auto err = msgpack(writer, field.name()))
			{
				return err;
			}
			if (auto err = field.write(writer))
			{
				return err;
			}
		}
	...
	inline HumanError struct_fields(Reader& reader, TField&&... fields)
	{
		...
		{
			uint16_t count{};
			if (auto err = reader.read_uint16(count))
			{
				return err;
			}
			fields_count = count;
		...
		{
			uint32_t count{};
			if (auto err = reader.read_uint32(count))
			{
				return err;
			}
			fields_count = count;
		...
		{
			if (auto err = msgpack(reader, field_name))
			{
				return err;
			}
	...
	template <typename T>
	inline static HumanError encode(Stream* stream, T& value, Allocator* allocator = nullptr)
	{
	...
	template <typename T>
	inline static HumanError decode(Stream* stream, T& value, Allocator* allocator = nullptr)
	{
