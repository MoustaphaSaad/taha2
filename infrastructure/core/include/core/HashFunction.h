...
namespace core
{
	inline static size_t _MurmurHashUnaligned2(const void* ptr, size_t len, size_t seed)
	{
		...
		else if constexpr (sizeof(void*) == 8)
		{
			...
			auto shift_mix = [](size_t v) -> size_t { return v ^ (v >> 47); };
			...
			for (const unsigned char* p = buffer; p != end; p += 8)
			{
	...
	inline static size_t fnva(Span<const std::byte> bytes, size_t seed = 0xc70f6907UL)
	{
	...
	struct Hash<float>
	{
		inline size_t operator()(float value, size_t seed) const
		{
			return value != 0.0f
					   ? hashBytes(
							 Span<const std::byte>{reinterpret_cast<const std::byte*>(&value), sizeof(value)}, seed)
					   : 0;
		}
	};
