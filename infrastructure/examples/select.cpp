...
{
	void consumerDefault(
		core::Shared<core::Chan<int>> evenNumbers,
		core::Shared<core::Chan<int>> oddNumbers,
		core::Log* log,
		core::Allocator* allocator)
	{
		...
		{
			core::Select{
				core::ReadCase{allocator, *evenNumbers} = [&](int num) { log->info("even consumer: {}"_sv, num); },
				core::DefaultCase{allocator} = [&] { log->info("default"_sv); },
				core::ReadCase{allocator, *oddNumbers} = [&](int num) { log->info("odd consumer: {}"_sv, num); },
			};
		}
	}
	...
	void test()
	{
		...
		core::Thread evenNumberProducerThread{
			&allocator, [evenNumbers, &log] { evenIntegerProducer(evenNumbers, &log); }};
		...
		core::Thread consumerThread{
			&allocator, core::Func<void()>{&allocator, [evenNumbers, oddNumbers, &log, &allocator] {
											   consumer(evenNumbers, oddNumbers, &log, &allocator);
										   }}};
...
namespace two_producers_fast_slow
{
	...
	void test()
	{
		...
		core::Thread evenNumberProducerThread{
			&allocator,
			[evenNumbers, &log] { slowEvenIntegerProducer(evenNumbers, &log, std::chrono::milliseconds{100}); }};
...
namespace timeout
{
	...
	void consumer(
		core::Shared<core::Chan<int>> numbers,
		core::Shared<core::Chan<bool>> stop,
		core::Log* log,
		core::Allocator* allocator)
	{
		core::Select{
			core::ReadCase{allocator, *numbers} = [&](int num) { log->info("number: {}"_sv, num); },
			core::ReadCase{allocator, *stop} = [&](bool stop) { log->debug("stop"_sv); },
		};
	}
	...
	void test()
	{
		...
		core::Thread evenNumberProducerThread{
			&allocator,
			[evenNumbers, &log] { slowEvenIntegerProducer(evenNumbers, &log, std::chrono::milliseconds{15}); }};

		auto stop = core::Chan<bool>::create(0, &allocator);
		core::Thread stopThread{&allocator, [stop, &log] { signalTimeout(stop); }};

		core::Thread consumerThread{&allocator, core::Func<void()>{&allocator, [evenNumbers, stop, &log, &allocator] {
																	   consumer(evenNumbers, stop, &log, &allocator);
																   }}};
...
namespace load_balancer
{
	...
	void balanceLoad(
		core::Shared<core::Chan<int>> tasks,
		core::Shared<core::Chan<int>> worker1,
		core::Shared<core::Chan<int>> worker2,
		core::Allocator* allocator)
	{
		for (auto task: *tasks)
		{
	...
	void test()
	{
		...
		core::Thread loadBalancer{
			&allocator, core::Func<void()>{&allocator, [evenNumbers, worker1Chan, worker2Chan, &allocator] {
											   balanceLoad(evenNumbers, worker1Chan, worker2Chan, &allocator);
										   }}};
