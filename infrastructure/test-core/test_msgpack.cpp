...
TEST_CASE("msgpack: string")
{
	...
	REQUIRE(bytes == "[0xbf, 0x31, 0x32, 0x33, 0x34, 0x35, 0x36, 0x37, 0x38, 0x39, 0x30, 0x31, 0x32, 0x33, 0x34, 0x35, 0x36, 0x37, 0x38, 0x39, 0x30, 0x31, 0x32, 0x33, 0x34, 0x35, 0x36, 0x37, 0x38, 0x39, 0x30, 0x31]"_sv);

	bytes = encode("12345678901234567890123456789012"_sv, &allocator);
	REQUIRE(bytes == "[0xd9, 0x20, 0x31, 0x32, 0x33, 0x34, 0x35, 0x36, 0x37, 0x38, 0x39, 0x30, 0x31, 0x32, 0x33, 0x34, 0x35, 0x36, 0x37, 0x38, 0x39, 0x30, 0x31, 0x32, 0x33, 0x34, 0x35, 0x36, 0x37, 0x38, 0x39, 0x30, 0x31, 0x32]"_sv);

	bytes = encode("Кириллица"_sv, &allocator);
	REQUIRE(bytes == "[0xb2, 0xd0, 0x9a, 0xd0, 0xb8, 0xd1, 0x80, 0xd0, 0xb8, 0xd0, 0xbb, 0xd0, 0xbb, 0xd0, 0xb8, 0xd1, 0x86, 0xd0, 0xb0]"_sv);
...
TEST_CASE("msgpack: array")
{
	...
	for (int i = 0; i < 15; ++i) medium.push(i + 1);
	core::Array<int> big{&allocator};
	for (int i = 0; i < 16; ++i) big.push(i + 1);
	...
	REQUIRE(bytes == "[0xdc, 0x0, 0x10, 0x1, 0x2, 0x3, 0x4, 0x5, 0x6, 0x7, 0x8, 0x9, 0xa, 0xb, 0xc, 0xd, 0xe, 0xf, 0x10]"_sv);
